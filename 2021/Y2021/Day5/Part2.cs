...
namespace Y2021.Day5
{
    public class Part2
    {
        public class Line
        {
            ...
            private void DrawDiagonalLine(Point start, Point end)
            {
                var incrementX = start.X < end.X ? 1 : -1;
                var incrementY = start.Y < end.Y ? 1 : -1;

                var y = 0;
                for (var x = start.X; x != end.X; x += incrementX)
                {
                    PointsOnTheLine.Add(new Point(x, start.Y + incrementY * y++));
                }
                PointsOnTheLine.Add(end);
            }
        }
