    ...
    public class Part2
    {
        ...
        public void Solution()
        {
            ...
            foreach (var line in input)
            {
                ...
                var one = dataInput.Where(x => x.Length == 2).First().ToArray();

                //I know 7 is build from 3 eleemnts;
                var seven = dataInput.Where(x => x.Length == 3).First().ToArray();

                //I know that upperRight and lowerRight elements are common for those digits so I am able to figure out upper element
                var upper = seven.Except(one.ToArray()).First();

                //I know that digit 4 is build from 4 elements
                var four = dataInput.Where(x => x.Length == 4).First().ToArray();

                //If I know upper value plus all values used to build digit 4. I am able to find 9 and lower part
                var check = four.ToList();
                check.Add(upper);

                var nine = dataInput.Where(x => x.Length == 6 && x.Except(check).Count() == 1).First().ToArray();
                var lower = nine.Except(check).First();

                //If I know nine then I will be able to find 8 and lowerLeft part
                var eight = dataInput.Where(x => x.Length == 7).First().ToArray();
                var lowerLeft = eight.Except(nine.ToArray()).First();
                ...
                var upperLeft = eight.Except(check).First();
