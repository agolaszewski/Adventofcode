...
namespace Y2021.Day4
{
    public class Part2
    {
        ...
        public void Solution()
        {
            ...
            {
                for (var boardNo = 0; boardNo < boards.Count;)
                {
                    if (FindWinningBoard(boards[boardNo], checkBoards[boardNo], luckyNumber))
                    {
                        if (boards.Count == 1)
                        {
                            var result = CalculateResult(boards[0], checkBoards[0], luckyNumber);
                            _console.WriteLine(result.ToString());
                            return;
                        }

                        boards.RemoveAt(boardNo);
                        checkBoards.RemoveAt(boardNo);
                    }
                    else
                    {
                        boardNo++;
        ...
        private bool FindWinningBoard(int[][] board, bool[,] checkBoard, int luckyNumber)
        {
            var isWinning = false;
            for (int x = 0; x < 5; x++)
            {
                for (int y = 0; y < 5; y++)
                {
                    if (board[x][y] == luckyNumber)
                    {
                        checkBoard[x, y] = true;
                        if (CheckColumn(checkBoard, x) || CheckRow(checkBoard, y))
                        {
                            isWinning = true;
                        }
                    }
